//! Aggregate per-client BM-BE4 multibench reports into fleet totals.
...
pub fn aggregate_be4(
    ...
    if groups.is_empty() {
        bail!(
            "no multibench BM-BE4 reports in {} (prefix={cell_prefix:?})",
            reports_dir.display()
        );
    }
    ...
    for (key, clients) in groups {
        ...
        for i in 0..count {
            ...
            if rate <= 0.0 {
                bail!("zero achieved rate for bench_client_index={i} ({fname})");
            }
            total += rate;
            per_client.push(json!({"bench_client_index": i, "ops_per_sec": rate, "report_file": fname}));
        }
        ...
        let aggregate = json!({
            ...
            "pass": true,
            "status": "ok",
            "notes": format!("multibench aggregate bc={count} fleet_size={} total={total:.0}/s", key.fleet_size),
        });
        std::fs::write(&out_path, serde_json::to_string_pretty(&aggregate)?)?;
        println!(
            "aggregate bc={count} fleet_size={}: {:.0} ops/s -> {}",
            key.fleet_size,
            total,
            out_path.display()
        );
