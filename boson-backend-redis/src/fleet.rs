//! Multi-broker Redis fleet: route logical pools to standalone Redis nodes.
...
use crate::RedisQueueBackend;

/// Routes `pool_i` → Redis broker `urls[i % N]` (standalone nodes).
pub struct PoolRoutedBackend {
    backends: Vec<Arc<RedisQueueBackend>>,
...
/// Parse fleet URLs from `BOSON_REDIS_URLS` or `BOSON_REDIS_POOL_ROUTING`.
pub fn fleet_urls_from_env() -> Result<Vec<String>> {
    if let Ok(routing) = std::env::var("BOSON_REDIS_POOL_ROUTING") {
        if !routing.trim().is_empty() {
            ...
            for part in routing.split(',') {
                ...
                let (_, url) = part.split_once('=').ok_or_else(|| {
                    BosonError::Backend(format!("invalid BOSON_REDIS_POOL_ROUTING segment: {part}"))
                })?;
    ...
    if let Ok(urls) = std::env::var("BOSON_REDIS_URLS") {
    ...
    Err(BosonError::Backend(
        "BOSON_REDIS_URLS or BOSON_REDIS_POOL_ROUTING required for fleet".into(),
    ))
}

/// Connect one Redis backend per fleet URL.
///
/// # Errors
///
...
pub async fn connect_fleet_from_env() -> Result<Arc<dyn QueueBackend>> {
    ...
    for url in &urls {
        backends.push(Arc::new(
            RedisQueueBackend::connect_with_keyspace(url, keyspace.clone()).await?,
        ));
    }

    if backends.len() == 1 {
        let backend = backends.into_iter().next().ok_or_else(|| {
            BosonError::Backend("expected one redis backend after connect".into())
        })?;
