#!/usr/bin/env bash
# Optional RAFT cluster comparison: provision 4 brokers, bootstrap cluster, run BE4 K=1.
set -euo pipefail
...
export BOSON_NATIVE_MANIFEST=boson-nats-cluster-4
export BOSON_NATIVE_CAMPAIGN="${BOSON_NATIVE_CAMPAIGN:-boson-cluster-n4-$(date -u +%Y%m%d)}"
...
echo "RAFT n=4 cluster BE4 publisher sweep done"
